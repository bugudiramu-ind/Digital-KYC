...
class Dashboard extends Component {
        ...
        axios(JSONData.fs_api_url + '/usage_stats', {
            ...
            if (res.status === 200) {
                ...
                for(var i in this.state.monthly_usage) {
                    ...
                    var str = this.state.monthly_usage[i]._id.month.substring(0,3) + " " +this.state.monthly_usage[i]._id.year; 
    ...
    userTable() {
        ...
        for (var i in this.state.usr_count) {
            tab.push(
                <tr key={i}>
                    <td>{this.state.usr_count[i].email.substring(0, this.state.usr_count[i].email.indexOf('@'))}</td>
                                        ...
                                        <div id="dataVisualRow" className="row">
                                            <div id="dataVisualCol" className="">
                                                <div id="dataVisualInner">
                                                    ...
                                                    {/* <Chart /> */}
                                                    <BarChart width={700} height={300} data={this.state.list}
                                                        margin={{ top: 50, right: 10, left: 15, bottom: 0 }}>
                                                        <CartesianGrid vertical={false} stroke="#ebf3f0" />
                                                        <XAxis dataKey="name" />
                                                        <YAxis>
                                                            <Label value="Documents Processed" angle={-90} position="left" style={{ textAnchor: 'middle' }} />
                                                        </YAxis>
